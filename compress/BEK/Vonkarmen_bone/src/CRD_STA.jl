# Compressible rotating-disk base flow and local stability operators.
# Initial implementation: H. J. Zhang, Tianjin University, 2024-2025.
module CRD_BF

    export sol_baseflowODE,velocity,Cheb,ChebFinite,phi_var,f_q,T_var,Physical_Interpretation
 ...
 function Cheb(N; domain=:rational, ymax=40.0, clip=40.0)
        ...
        elseif domain == :rational
            ...
            for i in eachindex(y)
                if denominator[i] <= eps(Float64)
                    y[i] = clip
                else
                    y[i] = min(a * (1 + h[i]) / denominator[i], clip)
                end
            end
        else
            throw(ArgumentError("domain must be :rational or :finite"))
        end

        return D, D^2, y
 end

 ChebFinite(N; ymax=40.0) = Cheb(N; domain=:finite, ymax=ymax)


 function Physical_Interpretation(T,delt,Num)
        length(T) >= Num || throw(DimensionMismatch("T has fewer than Num entries"))
        z = zeros(Float64, Num)
        for i in 2:Num
            z[i] = z[i - 1] + 0.5 * delt * (T[i - 1] + T[i])
        end
        return z
 end
 function phi_var(u,del,N)
    length(u) >= N || throw(DimensionMismatch("u has fewer than N entries"))
    phi = zeros(Float64, N)
    for i in 2:N
        phi[i] = phi[i - 1] + 0.5 * del * (u[i - 1] + u[i])
    end
    return phi
 end
...
const _BASEFLOW_PROFILE_CACHE = Dict{Float64,Any}()

function baseflow_var(N_cheb,Ro,Co; domain=:rational, ymax=40.0)
    N = 10001
    tspan = (0,40)
    t = range(0,40,N)
    sigma = 0.72
    cache_key = Float64(Ro)
    u0,v0,w0,f,q = get!(_BASEFLOW_PROFILE_CACHE,cache_key) do
        raw_u,raw_v,raw_w,du0,dv0,_ = CRD_BF.sol_baseflowODE(Ro)
        phi = CRD_BF.phi_var(raw_u,step(t),N)
        raw_u,du0,raw_v,dv0,raw_w,F_u,F_du,F_dv,_,F_phi = CRD_BF.velocity(
            raw_u,raw_v,raw_w,du0,dv0,t,phi,
        )
        ...
        (raw_u,raw_v,raw_w,f,q)
    end
    D,D2,x = CRD_BF.Cheb(N_cheb; domain=domain, ymax=ymax)
...
function interp(u,v,w,Tbase,x,N,mode)
    length(x) == N + 1 || throw(DimensionMismatch("x must contain N+1 points"))
    source_coordinate = if mode == "sim"
        range(0.0, 40.0, length=10001)
    elseif mode == "phy"
        CRD_BF.Physical_Interpretation(Tbase, 0.004, 10001)
    else
        throw(ArgumentError("mode must be \"sim\" or \"phy\""))
    end

    itu = BSplineKit.interpolate(source_coordinate, vec(u), BSplineOrder(4))
    itv = BSplineKit.interpolate(source_coordinate, vec(v), BSplineOrder(4))
    itw = BSplineKit.interpolate(source_coordinate, vec(w), BSplineOrder(4))
    itT = BSplineKit.interpolate(source_coordinate, vec(Tbase), BSplineOrder(4))
    points = vec(x)

    F = reshape(itu.(points), N + 1, 1)
    G = reshape(itv.(points), N + 1, 1)
    H = reshape(itw.(points), N + 1, 1)
    T = reshape(itT.(points), N + 1, 1)
    rho = 1 ./ T
    return F,G,H,T,rho,points
end

function Timemode_legacy(F,G,H,rho,lam,kappa,T,sigma,gamma,R,Ma,al,be,N_cheb,Ro,Co,D,D2)
        ...
        A0_omega_coeff_54 = -((gamma - 1) / gamma) * im * R .* T .* I(N_cheb + 1)
        A0_omega_coeff_55 = (im * R / gamma) .* rho .* I(N_cheb + 1)
        ...
        n = N_cheb + 1
        removed_rows = (n, n + 1, 2n, 2n + 1, 3n, 3n + 1, 4n, 4n + 1, 5n)
        removed_columns = (1, n, n + 1, 2n, 2n + 1, 3n, 4n, 4n + 1, 5n)
        retained_rows = setdiff(1:5n, removed_rows)
        retained_columns = setdiff(1:5n, removed_columns)
        B0 = A0[retained_rows, retained_columns]
        B1 = A0_omega_coeff[retained_rows, retained_columns]
     return B0,B1
end
...
function Timemode(F,G,H,rho,lam,kappa,T,sigma,gamma,R,Ma,al,be,N_cheb,Ro,Co,D,D2;
    ...
    coefficients = Spatial_mode_BEK(
        F,G,H,rho,lam,kappa,T,sigma,gamma,R,Ma,N_cheb,Ro,Co,D,D2;
        regularization=regularization,
        property_perturbations=property_perturbations,
        base_property_variation=base_property_variation,
    )
    L0,L1,L2 = assemble_mat(coefficients,D,D2,be,0.0)
    B0 = L0 + al .* L1 + al^2 .* L2
    B1 = im .* coefficients.Ta
    unused = zeros(ComplexF64,size(B0))
    B0,B1,_ = boundary_condition(B0,B1,unused,N_cheb)
    return B0,B1
end
...
"""
    Spatial_mode_BEK(...; property_perturbations=true,
                     base_property_variation=true)

Build the compressible stability coefficient tensors. `property_perturbations`
controls the Chapman-law disturbance terms `mu_hat=T_hat` and
`kappa_hat=T_hat/sigma`. `base_property_variation` controls whether
`mu_bar=T`, `lambda_bar=lambda`, and `kappa_bar=kappa` retain their profiles or
are frozen at their far-field values. Both switches modify only the stability
operator; the supplied base-flow profiles are never recomputed or replaced.
"""
function Spatial_mode_BEK(F,G,H,rho,lam,kappa,T,sigma,gamma,R,Ma,N_cheb,Ro,Co,D,D2;
    ...
    n = N_cheb + 1
    for (name, values) in (("T",T),("lambda",lam),("kappa",kappa))
        length(values) == n || throw(DimensionMismatch("$name must contain N_cheb + 1 values"))
    end

    # The published operator assumes Chapman's law mu=T. Keep thermodynamic
    # temperature separate from the material coefficients so that diagnostic
    # switches do not remove pressure, density, or energy coupling.
    mu_base = base_property_variation ? copy(T) : fill(T[end],size(T))
    lambda_base = base_property_variation ? copy(lam) : fill(lam[end],size(lam))
    kappa_base = base_property_variation ? copy(kappa) : fill(kappa[end],size(kappa))
    # lambda_hat multiplies div(u_bar)=0 and therefore has no linear term.
    property_scale = property_perturbations ? 1.0 : 0.0
    ...
    A_14 = R * F .* eye
    A_21 = R * rho .* F .* eye
    A_23 = - rho .* (D*mu_base) .* eye
    A_24 = (gamma*Ma^2)^(-1) * R .* T .* eye
    A_25 = (gamma*Ma^2)^(-1) * R .* rho .* eye
    A_32 = R * rho .* F .* eye
    A_41 = -rho .* (D*lambda_base) .* eye
    A_43 = R * rho .* F .* eye
    A_45 = -property_scale .* rho .* (D*F) .* eye
    A_53 = -2 * (gamma-1) * Ma^2 .* mu_base .* rho .* (D*F) .* eye
    ...
    B_33 = -rho .* (D*mu_base) .* eye
    dB_33 = -(D*rho) .* (D*mu_base) .* eye - rho .* (D2*mu_base) .* eye
    B_34 = (gamma * Ma^2)^(-1)  * R .* T .* eye
    dB_34 = (gamma * Ma^2)^(-1)  * R .* (D*T).*eye
    B_35 = (gamma * Ma^2)^(-1)  * R .* rho .* eye
    dB_35 = (gamma * Ma^2)^(-1)  * R .* (D*rho).*eye
    B_42 = -rho .* (D*lambda_base) .* eye
    dB_42 = -(D*rho) .* (D*lambda_base) .* eye - rho .* (D2*lambda_base) .* eye
    B_43 = R * rho .* G .* eye
    dB_43 = R * (D*rho) .* G .* eye + R * (D * G) .* rho .* eye
    B_45 = -property_scale .* rho .* (D*G) .* eye
    dB_45 = -property_scale .* ((D*rho).*(D*G) + rho .* (D2*G)) .* eye
    B_53 = -2 * (gamma-1) * Ma^2 .* mu_base .* rho .* (D*G) .* eye
    dB_53 = D * diag(B_53) .* eye
    ...
    C_51 = -2 * (gamma-1) * Ma^2 .* mu_base .* rho.^2 .* (D*F) .* eye
    # dC_51 = -2 * (gamma-1) * Ma^2 * (rho .* (D2*F) + (D*rho) .* (D*F)) .* eye
    dC_51 = D * diag(C_51) .* eye
    C_52 = -2 * (gamma-1) * Ma^2 .* mu_base .* rho.^2 .* (D*G) .* eye
    ...
    C_55 = (1/gamma * rho.^2 .* H +
            property_scale/sigma .* rho.^2 .* (D*T)) .* eye
    ...
    D_55 = (-(gamma-1)/gamma .* rho .* H .* (D*rho)
            - property_scale/sigma .* (rho .* (D*rho) .* (D*T) .+ rho.^2 .* (D2*T))
            - property_scale * (gamma-1) * Ma^2 .* rho.^2 .* ((D*F).^2 + (D*G).^2)) .* eye
    ...
    Vxx_14 = -regularization * eye
    Vxx_21 = -(lambda_base + 2*mu_base) .* eye
    Vxx_32 = -mu_base .* eye
    Vxx_43 = -mu_base .* eye
    Vxx_55 = -kappa_base .* eye
    ...
    Vyy_21 = -mu_base .* eye
    dVyy_21 = -(D*mu_base) .* eye
    d2Vyy_21 = -(D2*mu_base).*eye
    Vyy_32 = -(lambda_base .+ 2*mu_base) .* eye
    dVyy_32 = -(D*lambda_base .+ 2*D*mu_base) .* eye
    d2Vyy_32 = -(D2*lambda_base .+ 2*D2*mu_base) .* eye
    Vyy_43 = -mu_base .* eye
    dVyy_43 = -(D*mu_base).*eye
    d2Vyy_43 = -(D2*mu_base) .* eye
    Vyy_55 = -kappa_base .* eye
    dVyy_55 = -(D*kappa_base) .* eye
    d2Vyy_55 = -(D2*kappa_base) .* eye
    ...
    Vxy_22 = -(lambda_base .+ mu_base) .* eye
    dVxy_22 = -(D*lambda_base .+ D*mu_base) .* eye
    Vxy_31 = -(lambda_base .+ mu_base) .* eye
    dVxy_31 = -(D*lambda_base .+ D*mu_base) .* eye
    ...
    Vxz_23 = -rho .* (mu_base .+ lambda_base) .* eye
    # dVxz_23 = - (D*rho.*lam .+ rho.*D*lam) .* eye
    dVxz_23 = D * diag(Vxz_23) .* eye
    Vxz_41 = -rho .* (mu_base .+ lambda_base) .* eye
    ...
    Vyz_33 = -rho .* (mu_base .+ lambda_base) .* eye
    # dVyz_33 = - (D*rho.*lam .+ rho.*D*lam) .* eye
    dVyz_33 = D * diag(Vyz_33) .* eye
    Vyz_42 = -rho .* (mu_base .+ lambda_base) .* eye
...
boundary_condition(L0,L1,L2,N_cheb) = boudary_condition(L0,L1,L2,N_cheb)

function spatial_matrices(F,G,H,rho,lam,kappa,T,sigma,gamma,R,Ma,omega,be,
                          N_cheb,Ro,Co,D,D2; regularization=0.0,
                          property_perturbations::Bool=true,
                          base_property_variation::Bool=true)
    coefficients = Spatial_mode_BEK(
        F,G,H,rho,lam,kappa,T,sigma,gamma,R,Ma,N_cheb,Ro,Co,D,D2;
    ...
    L0,L1,L2 = assemble_mat(coefficients,D,D2,be,omega)
    return boundary_condition(L0,L1,L2,N_cheb)
end

function balance_polynomial(A0,A1,A2; iterations=2, scale_limit=1e12)
    iterations >= 0 || throw(ArgumentError("iterations must be nonnegative"))
    scale_limit > 1 || throw(ArgumentError("scale_limit must exceed one"))

    B0 = ComplexF64.(A0)
    B1 = ComplexF64.(A1)
    B2 = ComplexF64.(A2)
    n = size(B0,1)
    size(B0) == size(B1) == size(B2) ||
        throw(DimensionMismatch("all polynomial matrices must have the same size"))

    left_scale = ones(Float64,n)
    right_scale = ones(Float64,n)
    lower_limit = inv(scale_limit)

    scaling_step(values) = map(values) do value
        if isfinite(value) && value > 0
            clamp(inv(value),lower_limit,scale_limit)
        else
            1.0
        end
    end
    ...
    for _ in 1:iterations
        row_measure = max.(
            vec(maximum(abs.(B0),dims=2)),
            vec(maximum(abs.(B1),dims=2)),
            vec(maximum(abs.(B2),dims=2)),
        )
        row_step = scaling_step(row_measure)
        row_operator = reshape(row_step,:,1)
        B0 = row_operator .* B0
        B1 = row_operator .* B1
        B2 = row_operator .* B2
        left_scale .*= row_step
        ...
        column_measure = max.(
            vec(maximum(abs.(B0),dims=1)),
            vec(maximum(abs.(B1),dims=1)),
            vec(maximum(abs.(B2),dims=1)),
        )
        column_step = scaling_step(column_measure)
        column_operator = reshape(column_step,1,:)
        B0 = B0 .* column_operator
        B1 = B1 .* column_operator
        B2 = B2 .* column_operator
        right_scale .*= column_step
    end

    return B0,B1,B2,left_scale,right_scale
end

function polynomial_backward_error(A0,A1,A2,alpha,vector)
    vector_norm = norm(vector)
    vector_norm > 0 || return Inf
    residual = (A0 + alpha .* A1 + alpha^2 .* A2) * vector
    scale = (
        norm(A0) + abs(alpha) * norm(A1) + abs2(alpha) * norm(A2)
    ) * vector_norm
    return norm(residual) / max(scale,eps(Float64))
end

function mode_overlap(vector_a,vector_b; weight=nothing)
    length(vector_a) == length(vector_b) ||
        throw(DimensionMismatch("mode vectors must have the same length"))
    if isnothing(weight)
        denominator = norm(vector_a) * norm(vector_b)
        return denominator > 0 ? abs(dot(vector_a,vector_b)) / denominator : 0.0
    end

    weighted_a = weight * vector_a
    weighted_b = weight * vector_b
    denominator = sqrt(abs(dot(vector_a,weighted_a)) * abs(dot(vector_b,weighted_b)))
    return denominator > 0 ? abs(dot(vector_a,weighted_b)) / denominator : 0.0
end

function solve_spatial_mode(F,G,H,rho,lam,kappa,T,sigma,gamma,R,Ma,omega,be,
                            N_cheb,Ro,Co,D,D2;
                            target,neigs=1,initial_vector=nothing,
                            regularization=0.0,balance=true,
                            balance_iterations=2,maxit=800,tol=1e-10,
                            property_perturbations::Bool=true,
                            base_property_variation::Bool=true)
    neigs >= 1 || throw(ArgumentError("neigs must be at least one"))
    A0,A1,A2 = spatial_matrices(
        F,G,H,rho,lam,kappa,T,sigma,gamma,R,Ma,omega,be,
        N_cheb,Ro,Co,D,D2; regularization=regularization,
        property_perturbations=property_perturbations,
        base_property_variation=base_property_variation,
    )

    if balance
        B0,B1,B2,left_scale,right_scale = balance_polynomial(
            A0,A1,A2; iterations=balance_iterations,
        )
    else
        B0,B1,B2 = ComplexF64.(A0),ComplexF64.(A1),ComplexF64.(A2)
        left_scale = ones(Float64,size(A0,1))
        right_scale = ones(Float64,size(A0,1))
    end

    start_vector = if isnothing(initial_vector)
        ones(ComplexF64,size(A0,1))
    else
        length(initial_vector) == size(A0,1) ||
            throw(DimensionMismatch("initial_vector has the wrong length"))
        ComplexF64.(vec(initial_vector)) ./ right_scale
    end

    problem = PEP([B0,B1,B2])
    values,balanced_vectors = iar(
        problem;
        σ=ComplexF64(target),neigs=neigs,maxit=maxit,tol=tol,v=start_vector,
    )
    vectors = reshape(right_scale,:,1) .* balanced_vectors
    for column in axes(vectors,2)
        column_norm = norm(view(vectors,:,column))
        column_norm > 0 && (vectors[:,column] ./= column_norm)
    end

    overlaps = if isnothing(initial_vector)
        fill(NaN,length(values))
    else
        previous_mode = ComplexF64.(vec(initial_vector))
        [mode_overlap(previous_mode,view(vectors,:,index)) for index in eachindex(values)]
    end
    order = if isnothing(initial_vector)
        sortperm(eachindex(values),by=index -> abs(values[index] - target))
    else
        sortperm(
            eachindex(values),
            by=index -> (-overlaps[index],abs(values[index] - target)),
        )
    end
    values = values[order]
    ...
    residuals = [
        polynomial_backward_error(A0,A1,A2,values[index],vectors[:,index])
...
function Wcc_fun(z,N_cheb,)
    points = vec(z)
    length(points) == N_cheb + 1 ||
        throw(DimensionMismatch("z must contain N_cheb+1 points"))
    issorted(points) || throw(ArgumentError("z must be monotonically increasing"))

    Wcc = zeros(N_cheb + 1,N_cheb + 1)
    for i in 1:N_cheb + 1
        if i == 1
            Wcc[i,i] = (points[2] - points[1])/2
        elseif i == N_cheb + 1
            Wcc[i,i] = (points[i] - points[i-1])/2
        else
            Wcc[i,i] = (points[i+1] - points[i-1])/2
        end
    end
    W = kron(I(5),Wcc)
