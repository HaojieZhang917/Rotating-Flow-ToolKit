...
LAMBDA_C   = 1.0         # Boussinesq 离心浮力系数 beta*T_inf
ZMAX       = 20.0        # 计算域高度
...
def vonKarman_ODE(z, y):
    """
    动坐标系 Boussinesq von Kármán 方程
    y[0]=H, y[1]=F', y[2]=F, y[3]=G', y[4]=G, y[5]=T', y[6]=T
    """
    A = y[2]**2 + y[0]*y[1] - (y[4]-1.0)**2
    B = 2*y[2]*y[4] + y[0]*y[3] - 2*y[2]
    
    Fpp = A + LAMBDA_C * (y[6] - 1.0)
    Gpp = B
    return np.array(        [       
        -2.0 * y[2],        # H'  = -2F
        Fpp,      # F'' = F²+HF'-(G-1)²+lambda_c*(T-1)
        y[1],                # F'  = F'
        Gpp,      # G'' = 2FG+HG'-2F
        y[3],               # G'  = G'
        Pr * y[0] * y[5],   # T'' = Pr·H·T'
...
def solve_baseflow(Tw_target, verbose=True):
    ...
    # 使用 linspace 保证最后一个延拓点严格等于 Tw_target，且每步不超过 dTw。
    n_steps = max(1, int(np.ceil(abs(Tw_target - 1.0) / dTw)))
    tw_vals = np.linspace(1.0, Tw_target, n_steps + 1)[1:]
    ...
    # 延拓阶段使用较宽松容差；在准确目标壁温上按用户设置重新收敛。
    refined = solve_bvp(vonKarman_ODE, vonKarman_bc(Tw_target), z,
                        result.sol(z), tol=TOL, max_nodes=MAX_NODES)
    if not refined.success:
        if verbose:
            print(f"❌\n       最终高精度求解失败: {refined.message}")
        return None
    result = refined
...
def save_results(sol, Tw):
    ...
    header = (f"# vonKarman Boussinesq Tw={Tw:.4f} Pr={Pr} lambda_c={LAMBDA_C}\n"
    ...
    with open(jl_name, 'w', encoding='utf-8') as f:
        f.write(f"# Auto-generated by Bone.py\n")
        f.write(f"# Rotating-frame von Karman (Boussinesq)  Tw={Tw:.4f}  Pr={Pr}  lambda_c={LAMBDA_C}\n\n")
        # 标量
        f.write(f"const BF_Tw = {Tw:.6f}\n")
        f.write(f"const BF_Pr = {Pr:.6f}\n")
        f.write(f"const BF_lambda_c = {LAMBDA_C:.6f}\n")
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print(f"  Tw 序列: {Tw_list},  Pr = {Pr},  lambda_c = {LAMBDA_C},  zmax = {ZMAX}")
    print("=" * 60)
