module CRC_STA
    ...
    function assemble_adjmat(cof,D,D2,be,omega,R)
        # Keep the coefficient convention used by receptivity.ipynb cell 15.
        A0_raw = transpose(cof.D1) + (im * R * be * transpose(cof.B)) - (im * omega * transpose(cof.Ta)) - (be^2 * R * transpose(cof.Vyy)) - transpose(cof.dC) - (im * be * R * transpose(cof.dVyz)) + transpose(cof.d2Vzz) - (transpose(cof.C) + im * be * R * transpose(cof.Vyz) - 2 * transpose(cof.dVzz)) * kron(I(4),D) + transpose(cof.Vzz) * kron(I(4),D2)
    ...
    function boudary_condition(L0,L1,L2,N_cheb,mode)
        removed = (1,N_cheb + 1,N_cheb + 2,2N_cheb + 2,2N_cheb + 3,
                   3N_cheb + 3,4N_cheb + 4)
        keep = setdiff(1:size(L0, 1), removed)
        L0 = L0[keep, keep]
        L1 = L1[keep, keep]
        L2 = L2[keep, keep]
        return L0,L1,L2
    ...
    function eig_full(eigvec,N_cheb,num)
        N = N_cheb + 1
        v = eigvec[:, num]
        u_full = zeros(ComplexF64, N)
        v_full = zeros(ComplexF64, N)
        w_full = zeros(ComplexF64, N)
        p_full = zeros(ComplexF64, N)
        u_full[2:N-1] = v[1:N-2]
        v_full[2:N-1] = v[N-1:2N-4]
        w_full[2:N-1] = v[2N-3:3N-6]
        p_full[1:N-1] = v[3N-5:4N-7]
        return (u_full, v_full, w_full, p_full)
    end
end
