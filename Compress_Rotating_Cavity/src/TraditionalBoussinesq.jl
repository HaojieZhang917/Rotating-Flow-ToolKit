module TraditionalBoussinesq

using LinearAlgebra
using NPZ
using Statistics

export ThermalConfig, ThermalProfile, isothermal_profile, fixed_pressure_profile,
       fixed_temperature_profile, pressure_tangent, trace_principal_fold
...
# Chebyshev-Lobatto nodes on the rotor-stator gap 0 <= z <= 1.
...
"""
Traditional centrifugal Boussinesq closure for the finite-gap similarity flow.
...
The final unknown is Pi when Tw is fixed, or Tw when Pi is fixed. Retaining
Pi in radial momentum is essential for tracing through the Tw fold.
"""
function system(state::AbstractVector, config::ThermalConfig,
    ...
    residual = vcat(
        D * H + 2sqrt_re .* F,
        D2 * F - sqrt_re .* H .* DF - re .* (F.^2 .- G.^2 .+ T .- 1 .+ pi),
        D2 * G - sqrt_re .* H .* DG - 2re .* F .* G,
    ...
    J[rF, rH] .= -sqrt_re .* Diagonal(DF)
    J[rF, rF] .= D2 - sqrt_re .* Diagonal(H) * D - 2re .* Diagonal(F)
    J[rF, rG] .= 2re .* Diagonal(G)
    J[rF, rT] .= -re .* eye
    J[rG, rH] .= -sqrt_re .* Diagonal(DG)
    J[rG, rF] .= -2re .* Diagonal(G)
    J[rG, rG] .= D2 - sqrt_re .* Diagonal(H) * D - 2re .* Diagonal(F)
...
function newton(system_fn, initial, tolerance; max_iterations=24)
    ...
    for _ in 1:max_iterations
        ...
        if !accepted
            # Near a fold the Newton step can be smaller than roundoff while
            # the collocation residual is already below the requested scale.
            err < 10tolerance && return state, err
            error("thermal Newton line search failed; residual=$err")
        end
    end
    error("thermal Newton did not converge")
end

function solve(config::ThermalConfig, fields::AbstractMatrix,
    ...
    pi = fixed == :temperature ? state[end] : value
    tw = fixed == :pressure ? state[end] : value
    return ThermalProfile(z, D, unpack(view(state, 1:4length(z)), length(z)),
...
"""Start on the isothermal branch using the archived Re_h=1000 cavity profile."""
function isothermal_profile(config::ThermalConfig, reference_npz::AbstractString)
    z, D = chebyshev_grid(config.degree)
...
fixed_pressure_profile(pi::Real, seed::ThermalProfile) =
    solve(seed.config, seed.fields, seed.tw, :pressure, Float64(pi))

"""Solve the same branch at a prescribed wall temperature using a nearby profile as seed."""
fixed_temperature_profile(tw::Real, seed::ThermalProfile) =
    solve(seed.config, seed.fields, seed.pressure, :temperature, Float64(tw))

"""Return dTw/dPi from the implicit-function tangent along the branch."""
function pressure_tangent(profile::ThermalProfile)
    state = vcat(pack(profile.fields), profile.tw)
    _, J = system(state, profile.config, :pressure, profile.pressure, profile.D)
    n = length(profile.z)
    rhs = zeros(4n + 1)
    rhs[n+2:2n-1] .= profile.config.re_h  # minus partial residual / partial Pi
    return (J \ rhs)[end]
end

"""Continue from Tw=1 and refine the principal dTw/dPi=0 fold by bisection."""
function trace_principal_fold(isothermal::ThermalProfile;
                              coarse_step=0.003, fine_step=0.0005,
                              pressure_tolerance=1e-10)
    branch = ThermalProfile[isothermal]
    previous = isothermal
    # Coarse continuation is well away from the maximum; smaller steps resolve it.
    for pi in range(isothermal.pressure, 0.012; step=-coarse_step)[2:end]
        previous = fixed_pressure_profile(pi, previous)
        push!(branch, previous)
    end
    while previous.pressure > 0.0075
        pi = max(0.0075, previous.pressure - fine_step)
    ...
    error("principal fold was not bracketed")
end
